...
public class DoctorLogin  extends  JFrame implements ActionListener {
    ...
    JTextField username,password,userid;
...
DoctorLogin(){
    ...
    userid.setBounds(150,120,150,30);
    add(userid);
    ...
    cancel.setBounds(210,230,120,30);
    cancel.setBackground(Color.BLACK);
    cancel.setForeground(Color.WHITE);
    cancel.addActionListener(this);
    add(cancel);
    
    ImageIcon i1=new ImageIcon(ClassLoader.getSystemResource("images/download.jpeg"));
...
public void actionPerformed(ActionEvent ae){
       if(ae.getSource()==login){
           ...
           try{
             Conn c = new Conn();
             String query = "select * from doctor where userid = '"+id+ "'and password = '"+pass+"'";
             ResultSet rs=c.s.executeQuery(query);
             if(rs.next()){
                 
              setVisible(false);              
              DocDashboard db = new DocDashboard();
